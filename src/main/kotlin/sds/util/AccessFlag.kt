...
object AccessFlag {
    ...
    private fun getClassFlag(flag: Int): String  =
        build(flag, PUBLIC, STATIC, FINAL, SYNTHETIC, ABSTRACT) + when {
            checkAnd(flag, ANNOTATION.first) -> ANNOTATION.second
            checkAnd(flag, ENUM.first)       -> ENUM.second
            checkAnd(flag, INTERFACE.first)  -> INTERFACE.second
            else                             -> "class "
        }
    ...
    private fun build(target: Int, vararg flags: Pair<Int, String>): String =
        flags.filter { checkAnd(target, it.first) } .map { it.second }
             .reduce { flag1: String, flag2: String -> flag1 + flag2 }
